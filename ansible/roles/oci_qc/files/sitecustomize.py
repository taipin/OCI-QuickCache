...
import sys, os, pwd, json, yaml, hashlib, random, glob, time, csv, threading, boto3, botocore, re
...
OCI_QC_SHARD_PREFIX = CFG.get('OCI_QC_SHARD_PREFIX') or ""
OCI_QC_SHARD_FORM = CFG.get('OCI_QC_SHARD_FORM') or "03d"
#OCI_QC_SHARDS_PER_NODE = int(CFG.get('OCI_QC_SHARDS_PER_NODE') or 4)
#OCI_QC_MAPPING_LOG = CFG.get('OCI_QC_MAPPING_LOG') or "/var/log/ociqc/mapping.log"
OCI_QC_LOG_FILE = os.getenv("OCI_QC_LOG_FILE") or CFG.get('OCI_QC_LOG_FILE') or "cache_log.csv"
...
def get_shard_map_from_disk():
    """Read-only access to the shard map file."""
    try:
        if os.path.exists(OCI_QC_SHARD_MAP_FILE):
            with open(OCI_QC_SHARD_MAP_FILE, 'r') as f:
                return json.load(f)
        else:
            log_error_event(f"get_shard_map_from_disk", "MAP_MISSING", OCI_QC_SHARD_MAP_FILE, "", 0)

    except Exception as e:
        log_error_event("get_shard_map_from_disk", "READ_FAILED", OCI_QC_SHARD_MAP_FILE, "", 0)
        #print(f"ERROR: [sitecustomize] Failed to read shard map: {e}")
    return None

def refresh_shard_state():
    """Updates the global shard mapping variables from disk."""
    global OCI_QC_SHARD_MAP, OCI_QC_NUM_SHARDS, OCI_QC_LAST_CHECK_TIME

    new_map = get_shard_map_from_disk()
    if new_map:
    ...
    OCI_QC_LAST_CHECK_TIME = time.time()

# Initial load during Python startup
refresh_shard_state()
...
def get_shard_details(bucket, key, region=None):
    ...
    if (now - OCI_QC_LAST_CHECK_TIME) > OCI_QC_MAP_RELOAD_INTERVAL:
        refresh_shard_state()

    if OCI_QC_SHARD_MAP is None:
        log_error_event(f"s3://{bucket}/{key}", "MAP_IS_NONE", OCI_QC_SHARD_MAP_FILE, "", 0)
        return "", "", 0, False
    ...
    mount_path = OCI_QC_SHARD_MAP.get(str(shard_idx))

    if not mount_path:
        log_error_event(f"s3://{bucket}/{key}", "SHARD_INDEX_NOT_FOUND", OCI_QC_SHARD_MAP_FILE, url_hash, shard_idx)
        return "", url_hash, shard_idx, False
    ...
    full_path = os.path.join(shard_dir, region, bucket, key_dir, key_file)
...
def _infer_region_from_endpoint(client):
    """
    Prefer region parsed from the real endpoint host.
    This avoids profile/default-region drift when endpoint_url is overridden.
    """
    endpoint_candidates = []
    endpoint_obj = getattr(client, "_endpoint", None)
    endpoint_host = getattr(endpoint_obj, "host", None)
    meta_endpoint = getattr(client.meta, "endpoint_url", None)

    if endpoint_host:
        endpoint_candidates.append(endpoint_host)
    if meta_endpoint:
        endpoint_candidates.append(meta_endpoint)

    for endpoint in endpoint_candidates:
        parsed = urlparse(endpoint)
        host = (parsed.netloc or parsed.path or endpoint).split(":")[0].lower()
        m = re.search(r'(?:^|\.)compat\.objectstorage\.([a-z0-9-]+)\.oraclecloud\.com$', host)
        if not m:
            m = re.search(r'(?:^|\.)objectstorage\.([a-z0-9-]+)\.oraclecloud\.com$', host)
        if not m:
            # AWS S3 endpoint patterns:
            # - s3.<region>.amazonaws.com
            # - s3-<region>.amazonaws.com
            # - s3.dualstack.<region>.amazonaws.com
            # plus .com.cn variants
            m = re.search(
                r'(?:^|\.)(?:s3(?:[.-]dualstack)?[.-])([a-z0-9-]+)\.amazonaws\.com(?:\.cn)?$',
                host,
            )
        if m:
            return m.group(1)
...
def _get_effective_region(client):
    return _infer_region_from_endpoint(client) or getattr(client.meta, "region_name", None)
...
def patched_make_api_call(self, operation_name, kwarg):
    ...
    if operation_name == 'GetObject' and self.meta.service_model.service_name == 's3':
        ...
        region = _get_effective_region(self)
